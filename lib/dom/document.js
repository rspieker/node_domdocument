 ...
 *  @type    method
 *  @access  public
 *  @return  DocumentFragment instance
 */
Document.prototype.createDocumentFragment = function()
{
	var simple = factory.create(11, '#document-fragment');
	this.___simple(function(){
		simple.owner = this;
	});
...
Document.prototype.createElementNS = function(namespaceURI, qualifiedName)
{
	...
	else if (namespaceURI && part.length <= 1 && part[0] !== 'xmlns')
		throw new DOMException('NAMESPACE_ERR');
	//  if the prefix is 'xml', the namespaceURL must be http://www.w3.org/XML/1998/namespace
	else if (part[0] === 'xml' && namespaceURI !== 'http://www.w3.org/XML/1998/namespace')
		throw new DOMException('NAMESPACE_ERR');
	//  if the qualified name is 'xmlns', the namespace must be http://www.w3.org/2000/xmlns/
	else if (qualifiedName === 'xmlns' && namespaceURI !== 'http://www.w3.org/2000/xmlns/')
		throw new DOMException('NAMESPACE_ERR');

	simple = factory.create(1, qualifiedName);

	this.___simple(function(){
		simple.owner = this;
	});

	if ((namespaceURI || '') !== '' && part.length)
		simple.append(factory.create(2, 'xmlns:' + part[0], namespaceURI));
...
Document.prototype.adoptNode = function(source)
{
	...
	source.___simple(function(){
		var find = this.find(null, null, true),
			self = this;
