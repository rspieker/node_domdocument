...
Element.prototype.getAttributeNS = function(namespaceURI, localName)
{
	return this.___simple(function(){
		...
		this.data.forEach(function(attr){
			...
			Object.keys(attr.xmlns).forEach(function(prefix){
				if (attr.xmlns[prefix] === namespaceURI)
					match = prefix;
			});

			if (match && attr.name === (match !== '' ? match + ':' : '') + localName)
				result = attr.data;
		});
...
Element.prototype.setAttributeNS = function(namespaceURI, qualifiedName, value)
{
	...
	this.___simple(function(){
		...
		this.data.push(factory.create(2, qualifiedName, value, ns));
	});
};
