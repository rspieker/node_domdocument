...
{
	var name = this.___simple(function(){
			return this.qualifiedName(namespaceURI, localName);
		});
...
Element.prototype.setAttributeNS = function(namespaceURI, qualifiedName, value)
{
	...
	else if (namespaceURI && index < 0 && qualifiedName !== 'xmlns')
		throw new DOMException('NAMESPACE_ERR');
	//  if the prefix is 'xml', the namespaceURL must be http://www.w3.org/XML/1998/namespace
	else if (prefix === 'xml' && namespaceURI !== 'http://www.w3.org/XML/1998/namespace')
		throw new DOMException('NAMESPACE_ERR');
	...
	this.___simple(function(){
		...
		if (!(prefix === '' && namespaceURI === 'http://www.w3.org/2000/xmlns/'))
		{
			if ('undefined' === typeof ns[prefix])
				this.data.push(factory.create(2, 'xmlns:' + prefix, namespaceURI));
			ns[prefix] = namespaceURI;
		}

		this.data.push(factory.create(2, qualifiedName, value));
	});
};
...
Element.prototype.hasAttributeNS = function(namespaceURI, localName)
{
	...
	return name ? this.hasAttribute(name) : false;
};
