...
//			console.log('TODO: startDoctypeDecl (waiting for node-expat to implment this)', arguments);
		});

		parser.on('endDoctypeDecl', function(){
//			console.log('TODO: endDoctypeDecl (waiting for node-expat to implment this)', arguments);
		});
...
//			console.log('TODO: entityDecl', arguments);
	...
	sax.stream = function(file, done, encoding)
	{
		...
		parser.once('error', function(message){
			stream.close();
			error = new Error(message);
		});

		stream.on('data', function(data){
//			var match = first ? data.match(/^\s*<!doctype([^>]*)>/i) : false;
			first = false;
