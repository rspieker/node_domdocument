...
function SimpleSerializer()
{
	 ...
	 */
	function encode(character, options)
	{
		return character in options.entity ? options.entity[character] : '&#' + character.charCodeAt() + ';';
	}
	...
	function quoteAttribute(value, format)
	{
		...
		if (isHTML(format) && pattern.quoteLessAttribute.test(value))
			quote = '';
	...
	/*
	 *  Obtain all namespace declarations which only reside in the current node
	 *  @name   getOwnXMLNS
	 *  @type   function
	 *  @access internal
	 *  @param  Simple node
	 ...
	 */
	function getOwnXMLNS(node)
	{
		var xmlns = node.ns(),
			parent = node.parent ? node.parent.ns() : {};

		//  remove anything which is not already defined in the parent
		Object.keys(parent).forEach(function(key){
			if (xmlns[key] === parent[key])
				delete xmlns[key];
		});
	...
	function serializeToString(node, buffer, options)
	{
		...
		switch(node.type)
		{
			case serializer.ELEMENT_NODE:
				...
				value = getOwnXMLNS(node);

				Object.keys(value)
					.sort()
					.forEach(function(key){
						if (key && key !== '')
							buffer.push(' xmlns:', key, '="', value[key], '"');
					})
				;
			...
			case serializer.ATTRIBUTE_NODE:
				if (node.name.indexOf('xmlns') !== 0)
				{
					if (isAutoFill(node.name, options.format))
						return buffer.push(' ', node.name);
					else if (node.data === '' && options.removeEmptyAttributes && !/^data-/i.test(node.name))
						return;
			...
			case serializer.TEXT_NODE:
				value = node.data.replace(pattern.encode, function(character){
					return encode(character, options);
				});
				if (!pattern.preserveWhiteSpace.test(node.parent.name) && !options.preserveWhiteSpace)
			...
			case serializer.DOCUMENT_TYPE_NODE:
				if (!options.format)
					options.format = detectFormat(node.data.publicId || node.name);

				buffer.push('<!DOCTYPE ', node.name);

				if (node.data.publicId)
					buffer.push(' PUBLIC "', node.data.publicId, '"');

				if (node.data.systemId && node.data.systemId !== '.')
				{
					if (!node.data.publicId)
						buffer.push(' SYSTEM');
					buffer.push(' "', node.data.systemId, '"');
				}

				//if (node.data.internalSubset)
				//	buffer.push(' [', node.data.internalSubset, ']');
			...
			case serializer.PROCESSING_INSTRUCTION_NODE:
				if (!options.format)
					options.format = detectFormat(node.name);
