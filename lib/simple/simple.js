...
Simple.prototype.ns = function(scope)
{
	var xmlns = scope || {},
		prefix;

	//  if we are dealing with DOMNodes (type 1), we need to ensure all xmlns(:[a-z]) attributes rub off on us
	if (this.type === 1)
		this.data.forEach(function(attr){
			if (attr.name.indexOf('xmlns') === 0 && !(attr.data in xmlns))
			{
				prefix = attr.name === 'xmlns' ? '' : attr.name.substr(6);
				if (!(prefix in xmlns))
					xmlns[prefix] = attr.data;
			}
		});

	//  provide any namespace directly associated with this Simple
	if (this.xmlns)
		Object.keys(this.xmlns).forEach(function(key){
			if (!(key in xmlns))
				xmlns[key] = this.xmlns[key];
		});

	//  any element which is not an attribute should inherit from its parent, if any
	if (this.type !== 2 && this.parent)
		this.parent.ns(xmlns);
